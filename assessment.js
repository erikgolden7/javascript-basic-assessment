...
// #6 Loop through this object and change all keys that start with the letter s to have a value of 's'
  ...
  for(var key in snake){
      if(snake.hasOwnProperty(key) && key.charAt(0) === 's'){
       	snake[key] = "s";
      }
   }
...
function coderTest(personObj) {
  if (personObj.hasOwnProperty('name')) {
...
function outside(temperature, humidity, cloud) {
	if (temperature > 80 && humidity > 40){
		return "I'm all sweat";
	}
	else if (temperature < 40 && cloud > 60) {
		return "I have icecicles";
	}
	else if (temperature > 80 && humidity < 40 && cloud < 20) {
		return "I'm literally in the desert";
	}
	else if (temperature > 80 || humidity > 50 || cloud > 50) {
